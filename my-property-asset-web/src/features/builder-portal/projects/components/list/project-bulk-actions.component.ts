...
import { ProjectBulkAction } from '../../models/project.model';

@Component({
  selector: 'app-proj-bulk-actions',
  imports: [ButtonComponent],
  template: `
    @if (count() > 0) {
      <div class="proj-bulk-actions" role="toolbar" aria-label="Bulk actions">
        <span class="proj-bulk-actions__count">{{ count() }} selected</span>
...
export class ProjectBulkActionsComponent {
  readonly count = input(0);

  readonly action = output<ProjectBulkAction>();
