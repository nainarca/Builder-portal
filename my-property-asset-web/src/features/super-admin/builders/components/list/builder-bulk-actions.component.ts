...
import { BuilderBulkAction } from '../../models/builder-admin.model';

@Component({
  selector: 'app-bldr-bulk-actions',
  imports: [ButtonComponent],
  template: `
    @if (count() > 0) {
      <div class="bldr-bulk-actions" role="toolbar" aria-label="Bulk actions">
        <span class="bldr-bulk-actions__count">{{ count() }} selected</span>
...
export class BuilderBulkActionsComponent {
  readonly count = input(0);
  readonly action = output<BuilderBulkAction>();
